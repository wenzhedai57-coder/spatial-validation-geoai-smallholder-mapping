...
- Do not add any row to the future independently reviewed reference set until it has a final label and an explicit inclusion decision. The historical `verified` field, if retained for schema compatibility, must be treated only as that inclusion flag.
...
Rubber must be prioritized because the author-resolved-subset rerun leaves only 16 rubber samples. Other visually confusable agricultural classes should also be prioritized if they remain under-supported within key folds or regions. Oversampling is acceptable only if the inclusion probabilities and design weights are recorded.
...
## Step 8. Create the future independently reviewed reference file
