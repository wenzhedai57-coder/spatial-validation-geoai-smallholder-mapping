"""Summarize author-resolved-subset sensitivity outputs for manuscript revision."""
...
def main() -> int:
    ...
    required = {
        "class_counts": results_dir / "author_resolved_reference_class_counts_20260618.csv",
    ...
    for stack in ["B2", "B3"]:
        for metric in ["oa", "macro_f1"]:
            ...
            metric_rows.append(
                {
                    "timestamp": timestamp,
                    "scope": "author_resolved_subset_table4_random_vs_spatial",
                    "stack": stack,
                    "model": "RandomForest",
                    "metric": metric,
                    **values,
    ...
    for _, row in rubber_transfer.iterrows():
        ...
        transfer_rows.append(
            {
                "timestamp": timestamp,
                "scope": "author_resolved_subset_rubber_belt_transfer",
                "stack": row.get("stack", ""),
                "model": row.get("model", ""),
                "status": row.get("status", ""),
                "reason": row.get("reason", ""),
    ...
    summary_csv = results_dir / "author_resolved_sensitivity_manuscript_summary_20260618.csv"
    ...
    provenance = {
        ...
        "note": "All summary values are copied or computed from author-resolved-subset CSV/JSON outputs; no manuscript numbers were hand-entered.",
    }
    summary_json = results_dir / "author_resolved_sensitivity_manuscript_summary_20260618.json"
    ...
    (results_dir / "AUTHOR_RESOLVED_SENSITIVITY_MANUSCRIPT_SUMMARY_20260618.md").write_text(
        "\n".join(lines) + "\n", encoding="utf-8"
    )
