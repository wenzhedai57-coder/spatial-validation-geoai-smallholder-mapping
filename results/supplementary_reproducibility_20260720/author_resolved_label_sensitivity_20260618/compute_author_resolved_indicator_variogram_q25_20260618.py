"""Compute author-resolved-subset one-vs-rest indicator variograms and q25 block distance."""
...
def main() -> int:
    ...
    parser.add_argument("--config", default="config/config_author_resolved_label_sensitivity_20260618.yaml")
    ...
    results_dir = (root / config.get("paths", {}).get("results", "results/author_resolved_label_sensitivity_20260618")).resolve()
    figures_dir = (root / config.get("paths", {}).get("figures", "figures/author_resolved_label_sensitivity_20260618")).resolve()
    ...
    try:
        ...
        ax.set_title("Author-resolved-subset one-vs-rest indicator variograms")
        ax.grid(True, linewidth=0.3, alpha=0.5)
        ax.legend(fontsize=7)
        fig.tight_layout()
        fig.savefig(figures_dir / "author_resolved_variogram_indicator_ranges_20260618.png")
        fig.savefig(figures_dir / "author_resolved_variogram_indicator_ranges_20260618.pdf")
        plt.close(fig)
